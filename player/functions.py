...
def updatePlayer(player, i=None, n=None):
    ...
    if not player.active and not player.validKey:
        ...
        print(
            "[player.functions.updatePlayer] {}{} action: {:010} active: {:1} api: {:1} -> api error {}".format(
                progress, player.nameAligned(), player.lastActionTS, player.active, player.validKey, user["apiError"]
            )
        )
