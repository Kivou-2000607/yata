...
class Server(models.Model):
    ...
    def get_revive(self, page=None):
        ...
        all = [{"server_id": str(s.discord_id), "server_name": s.name, "public": get_revive_public(s)} for s in Server.objects.filter(bot=self.bot).order_by("name") if len(json.loads(s.configuration).get('revive', {}).get('channels_alerts', {})) and s != self]
        ...
        if from_db:
            for_template = {
                ...
                "other": {"type": "bool", "all": ["public"], "selected": from_db.get("other", []), "title": "Public server", "prefix": "", "help": "State to others that you accept their calls", "tooltip": "It will display a green tick in the public column of the revive servers table.", "mandatory": False},
            }
            return for_template
        else:
            return False
