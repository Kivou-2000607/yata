...
urlpatterns = [
    ...
    re_path(
        r"^configurations/key/$", views.configurationsKey, name="configurationsKey"
    ),
    re_path(
        r"^configurations/threshold/$",
        views.configurationsThreshold,
        name="configurationsThreshold",
    ),
    re_path(
        r"^configurations/poster/$",
        views.configurationsPoster,
        name="configurationsPoster",
    ),
    re_path(
        r"^configurations/event/$",
        views.configurationsEvent,
        name="configurationsEvent",
    ),
    # SECTION: members
    ...
    re_path(
        r"^chains/export/(?P<chainId>\w+)/(?P<type>\w+)$",
        views.reportExport,
        name="reportExport",
    ),
    # SECTION: attacks
    ...
    re_path(
        r"^attacks/members/(?P<reportId>\w+)$",
        views.attacksMembers,
        name="attacksMembers",
    ),
    re_path(
        r"^attacks/export/(?P<reportId>\w+)/(?P<type>\w+)$",
        views.attacksExport,
        name="attacksExport",
    ),
    ...
    re_path(
        r"^bigbrother/remove/$", views.removeContributors, name="removeContributors"
    ),
    # SECTION: territories
    re_path(r"^territories/$", views.territories, name="territories"),
    re_path(
        r"^territories/fullmap/$", views.territoriesFullMap, name="territoriesFullMap"
    ),
    # SECTION: territories
    re_path(r"^simulator/$", views.simulator, name="simulator"),
    re_path(
        r"^simulator/challenge/$", views.simulatorChallenge, name="simulatorChallenge"
    ),
    # SECTION: oc
    re_path(r"^oc/$", views.oc, name="oc"),
    re_path(r"^ocList/$", views.ocList, name="ocList"),
    # SECTION: oc2
    re_path(r"^oc2/$", views.oc2, name="oc2"),
    # SECTION: oc
    ...
    re_path(
        r"^spies/(?P<secret>\w+)/(?P<export>\w+)/$", views.spies, name="spiesExport"
    ),
    # SECTION: fight club
