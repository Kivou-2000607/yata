...
$(document).on('click', '#yata-logout-submit', function() {
    $(this).html(spinner);
});

$(document).on('click', '#yata-delete-submit', (e) => {
    const r = confirm("Are you sure you want to delete your account? All data will be removed from the database.");
    if (r == false) {
        e.preventDefault();
    }
});
...
$(document).on('click', '#yata-toggle-color-mode', function(e) {
  ...
  const cssFile = $(this).attr("data-css-file");
  document.querySelectorAll("head link[href*='/static/yata/css/css-vars-light']").forEach(x => x.remove());
  document.querySelectorAll("head link[href*='/static/yata/css/css-vars-dark']").forEach(x => x.remove());
  const fileref = $("<link>", {
    rel: "stylesheet",
    type: "text/css",
    href: cssFile
