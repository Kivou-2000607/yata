...
$(document).on(
  "click",
  "tr[id^='target-list-refresh-'] > td:not(.dont-touch-me)",
  (e) => {
    e.preventDefault();
...
$(document).on("click", "#target-refresh", (e) => {
  ...
  $("#target-targets")
    .find("tr[id^='target-list-refresh-']")
    .each((useless, e) => {
      // if ((!refresh_color || $(this).find("span.target-color-" + refresh_color).length) && $(this).attr("data-ref") == "1") {
      if (
        refresh_colors.includes(
          $(e).find("span.target-list-note-color").attr("data-col")
        ) &&
        +e.dataset.ref === 1
      ) {
        const reload = $(e);
        ...
        ((index) => {
          setTimeout(() => {
            ...
            reload.removeClass("old-refresh");
            reload.html(
              "<td colspan='15' class='text-center'><i class='fas fa-spinner fa-pulse p-1'></i></td>"
            );
          }, wait);
        })(i);
        i++;
...
$(document).on("click", "#target-list-refresh-color", (e) => {
  ...
  $("#target-refresh").html(
    `<i class="fas fa-sync-alt"></i>&nbsp;Refresh ${colors[color]}`
  );
  ...
  $("i[id^='target-list-refresh-color-']")
    .removeClass("fa-check-square")
    .addClass("fa-square");
  $(`#target-list-refresh-color-${color}`)
...
$(document).on("click", "#target-add-submit", (e) => {
  e.preventDefault();
  const ids = $("#target-add-id").val();
  const array = ids.split(",");
  array.forEach((id) => {
    $("#content-update").load("/target/target/", {
      targetId: id.replace(/\s/g, ""),
      type: "addById",
      csrfmiddlewaretoken: getCookie("csrftoken"),
    });
    $("#content-update h2").addClass("grey");
    $("#content-update h2").html(
      `${spinner}&nbsp;&nbsp;Adding target id ${id} (1 API call)`
    );
  });
});
