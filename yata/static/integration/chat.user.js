            ...
            onload: resp=> {
                ...
                if ("message" in obj) {
                    if ("type" in obj && obj.type>0) {
                        console.log("YATA SECRET: " + obj.message);
                        setCookie();
                    }
                    else alert("YATA SECRET: " + obj.message);
                } else alert("YATA SECRET: Error");
            },
            ontimeout: ()=>{
                alert("YATA SECRET: Request has timed out");
            },
            onerror: ()=>{
                alert("YATA SECRET: Unknown error has occured when trying to send the data");
            },
            onabort: ()=>{
                alert("YATA SECRET: Upon sending the data, the request was canceled");
            }
        });
    } else {
        console.log("YATA SECRET: secret already sent recently");
    }
})();
